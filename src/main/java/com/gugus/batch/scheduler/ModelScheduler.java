...
public class ModelScheduler {
    ...
    public void run() {
        if (!enabled) return;
        log.info("[ModelScheduler] Model sync job started");

        try {
            // 모든 브랜드-카테고리 조합에 대해 모델 동기화 실행
            var pairs = pairProvider.allPairsForModels();
            log.info("[ModelScheduler] Processing {} brand-category pairs", pairs.size());
            ...
            for (var pair : pairs) {
                try {
                    ...
                    log.error("[ModelScheduler] Model sync failed for {}-{}", pair.brandCode(), pair.categoryCode(), e);
                    failCount++;
                }
            }
            
            log.info("[ModelScheduler] Model sync job completed - Success: {}, Failed: {}", successCount, failCount);
        } catch (Exception e) {
            log.error("[ModelScheduler] Model sync job failed", e);
        }
    }
}
